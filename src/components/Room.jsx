...
export default function Room({ roomKey }) {
  ...
  const [phase, setPhase] = useState('connecting') // connecting | setup | login | in
  ...
  useEffect(() => {
    const offs = [
      conn.on('room-status', (m) => { if (!conn.creds) setPhase(m.exists ? 'login' : 'setup') }),
      conn.on('joined', (m) => { setJoined(m); setPhase('in'); setError(''); setWsDown(false) }),
      conn.on('error', (m) => {
        if (m.code === 'gone' && conn.creds) {
          conn.send({ type: 'create', password: conn.creds.password, name: conn.creds.name })
  ...
  if (phase === 'in' && joined) {
    return (
      <>
        ...
        <RoomShell key={joined.selfId} conn={conn} joined={joined} roomKey={roomKey} />
      </>
    )
  }

  return <Gate phase={phase} roomKey={roomKey} error={error} onSubmit={(mode, pw, name) => { setError(''); conn.auth(mode, pw, name) }} />
}

function Gate({ phase, roomKey, error, onSubmit }) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-2xl bg-slate-800/60 border border-slate-700 p-6">
          <h1 className="font-bold text-lg">
            {phase === 'connecting' ? 'Connecting…' : creating ? 'Create room' : 'Join room'}
          </h1>
          <p className="text-sm text-slate-400 mt-0.5 break-all">
            {phase === 'connecting' ? roomKey : creating
          ...
          {phase !== 'connecting' && (
            <form onSubmit={submit} className="mt-4 space-y-3">
              ...
              <input
                ...
                placeholder={creating ? 'Set a password' : 'Room password'}
                className="w-full rounded-lg bg-slate-900/70 border border-slate-700 px-3.5 py-3 outline-none focus:border-sky-500 placeholder:text-slate-500"
              />
              ...
              <button className="w-full rounded-lg bg-sky-500 hover:bg-sky-400 active:scale-[.98] transition py-3 font-semibold text-white">
                {creating ? 'Create & enter' : 'Join room'}
              </button>
            </form>
          )}
        </div>
